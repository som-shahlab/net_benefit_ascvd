...
def patient_split_cv(
    df, patient_col="person_id", test_frac=0.1, eval_frac=0.1, nfold=10, seed=657
):
    ...
    patient_df = df[[patient_col]].drop_duplicates()
    # Shuffle the patients
    patient_df = patient_df.sample(frac=1, replace=False, random_state=seed)
    ...
    # Get the number of patients in each fold
    test_patient_df = patient_df.iloc[0:num_test].assign(fold_id="test")
    eval_patient_df = patient_df.iloc[num_test : (num_eval + num_test)].assign(
        fold_id="eval"
    )

    train_patient_df = patient_df.iloc[(num_eval + num_test) :]
    ...
    patient_df = pd.concat(
        [train_patient_df, test_patient_df, eval_patient_df], ignore_index=True
    )
